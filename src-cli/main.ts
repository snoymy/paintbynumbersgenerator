...
async function main() {
  ...
  if (typeof imagePath === "undefined" || typeof svgPath === "undefined") {
    console.log(
      "Usage: exe -i <input_image> -o <output_svg> [-c <settings_json>]"
    );
    process.exit(1);
  }
  ...
  await ColorReducer.applyKMeansClustering(
    ...
    (kmeans) => {
      ...
      (ctxKmeans as unknown as CanvasRenderingContext2D).putImageData(
        kmeansImgData as unknown as ImageData,
        0,
        0
      );
  ...
  const colormapResult = ColorReducer.createColorMap(
    kmeansImgData as unknown as ImageData
  );
    ...
    const svgProfilePath =
      path.join(
        ...
        path
          ...
          .substr(
            0,
            path.basename(svgPath).length - path.extname(svgPath).length
          ) +
          "-" +
          profile.name
      ) +
  ...
  for (const f of facetResult.facets) {
    if (f != null && f.borderSegments.length > 0) {
      ...
      if (stroke) {
        ...
        if (fill) {
          svgStroke = `rgb(${colorsByIndex[f.color][0]},${
      ...
      let svgFill = "";
      if (fill) {
        svgFill = `rgb(${colorsByIndex[f.color][0]},${
          colorsByIndex[f.color][1]
        },${colorsByIndex[f.color][2]})`;
      } else {
        svgFill = "none";
